...
post '/upload' => sub {
  ...
  $self->db->txn(sub {
    ...
    my $sth = $_->prepare(q{
      INSERT INTO upload (user_id, artist, title, size, hash, info, quality, server, filename, upload_date)
        VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    });

    $sth->execute($req->id, @{$p}{qw{artist title size hash info quality server filename}}, time);
